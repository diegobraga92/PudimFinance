...
/// Request body for completing a Google OAuth authorization-code flow.
#[derive(Debug, Deserialize, utoipa::ToSchema)]
pub struct GoogleAuthRequest {
    ...
    /// Public OAuth client ID used for the authorization request.
...
pub async fn google_login(
    ...
    if !verifier
        .client_ids()
        .iter()
        .any(|id| id == &payload.client_id)
        || !valid_google_redirect(&payload.redirect_uri, &payload.client_id)
    {
        return Err((
            StatusCode::BAD_REQUEST,
    ...
    if claims.aud != payload.client_id {
        return Err((
            StatusCode::UNAUTHORIZED,
